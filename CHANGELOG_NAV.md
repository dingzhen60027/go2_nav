...
## 2026-07-16 - Uncommitted conservative recovery behavior tree

Status: uncommitted

Purpose:
- Improve Go2 stability during Nav2 recovery.
- Avoid large-angle recovery spins that can disturb ICP localization and create
  unstable robot motion.

Changes:
- Added short waits after context costmap clears so costmaps can repopulate
  before replanning or following.
- Replaced full recovery sequence:
  - old: clear local/global, spin `1.57 rad`, wait `5 s`, backup `0.30 m`
  - new: clear local + wait `1 s`, backup `0.18 m`, clear global + wait `1 s`,
    then small spin `0.30 rad`
- Kept a small spin only as a last-resort viewpoint change. Large-angle spin is
  no longer used.
...
ET.parse('nav2_config/navigate_to_pose_w_smoothing.xml')
print('xml ok')
PY`

## 2026-07-16 - Uncommitted stricter ICP tracking gate

Status: uncommitted

Purpose:
- Reduce accepted ICP pose jumps while the robot is moving.
- Favor stable TF during normal navigation over aggressive pull-in from a bad
  initial pose.

Changes:
- Tightened `fast_icp_loc` tracking gate:
  - `max_translation_delta: 0.45 -> 0.25`
  - `max_yaw_delta: 0.60 -> 0.35`
  - `max_fitness_score: 1.0 -> 0.75`

Tradeoff:
- This makes normal tracking less likely to jump, but also weakens the ability
  to pull back from a poor initial pose. A separate relocalization mode should
  use looser gates or multi-hypothesis matching.

Files:
- `src/fast_icp_loc/config/fast_icp_loc.yaml`

Validation:
- `python3 -c "import yaml; yaml.safe_load(open('src/fast_icp_loc/config/fast_icp_loc.yaml')); print('fast_icp yaml ok')"`

## 2026-07-16 - Uncommitted ICP initial convergence gate split

Status: uncommitted

Purpose:
- Keep strict ICP gates during normal navigation while allowing RViz
  `/initialpose` to pull into the map from a rough initial guess.
- Prepare the code structure for a future global relocalization mode that does
  not share tracking jump gates.

Changes:
- Split ICP acceptance logic into helper functions:
  - `inInitialConvergence()`
  - `shouldAcceptIcpResult(...)`
- Added an initial convergence window after `/initialpose`:
  - `init_convergence_frames: 10`
  - `init_max_fitness_score: 1.2`
- During this initial window, ICP results are not rejected by
  `max_translation_delta` or `max_yaw_delta`; only matching quality is checked.
- After the window, normal strict tracking gates apply:
