  ...
  const submit = async (event) => {
    ...
    try {
      if (stopFirst) {
        ...
        if (!stopped.current_capture?.available || stopped.current_capture.archived) {
  ...
  return (
    <Modal title={stopFirst ? '结束建图并保存 PCD' : '保存原始 PCD'} subtitle={capture?.file?.name || 'scans.pcd'} onClose={onClose}>
      <form onSubmit={submit} className="modal-form">
        ...
        <div className="source-summary"><HardDrive size={16} /><span>{capture?.file?.size_human}</span><code>{capture?.source_path}</code></div>
...
export default function App() {
  ...
  const [section, setSection] = useState('versions')
  ...
  const startRuntime = async (mode, mapVersion = null) => {
    setActionBusy(true)
    try {
      if (mapVersion && mapVersion.id !== data?.active?.id) {
        await api('/api/active', { method: 'POST', body: JSON.stringify({ version_id: mapVersion.id }) })
      }
      await api('/api/runtime/start', { method: 'POST', body: JSON.stringify({ mode }) })
      notify(mode === 'mapping' ? '建图已启动' : `${mode === 'localization' ? 'ICP 定位' : '导航'}已使用所选地图启动`)
      await refresh(true)
    } catch (requestError) {
      notify(requestError.message, 'error')
    } finally {
    ...
    <div className="app-shell">
      ...
      <section className="runtime-bar">
        <div className={`runtime-state ${data?.runtime?.status || 'idle'}`}>
          ...
          <div><strong>{data?.runtime?.status === 'running' ? `${{ mapping: '正在建图', localization: '定位运行中', navigation: '导航运行中' }[data.runtime.mode]}` : data?.runtime?.status === 'stopping' ? '正在停止' : data?.runtime?.status === 'failed' ? '流程异常退出' : '系统待命'}</strong><span>{data?.runtime?.status === 'running' || data?.runtime?.status === 'failed' ? data?.runtime?.logs?.at(-1) || '正在启动' : '当前没有运行 ROS 流程'}</span></div>
        </div>
        <div className="core-pipeline" aria-label="地图处理流程">
          <section className={`pipeline-stage ${data?.runtime?.mode === 'mapping' && data?.runtime?.status === 'running' ? 'running' : ''}`}>
            <div className="pipeline-title"><span>1</span><div><strong>建图</strong><small>输出：原始 PCD</small></div></div>
          ...
          <section className={`pipeline-stage run-stage ${['localization', 'navigation'].includes(data?.runtime?.mode) && data?.runtime?.status === 'running' ? 'running' : ''}`}>
            <div className="pipeline-title"><span>3</span><div><strong>使用地图</strong><small>{selected?.name || '先选择生成的地图'}</small></div></div>
            {['localization', 'navigation'].includes(data?.runtime?.mode) && ['running', 'stopping'].includes(data?.runtime?.status)
              ...
              : <div className="map-run-actions"><button onClick={() => startRuntime('localization', selected)} disabled={actionBusy || !selected?.complete || data?.job?.running || ['running', 'stopping'].includes(data?.runtime?.status)}><MapPin size={16} />ICP 定位</button><button onClick={() => startRuntime('navigation', selected)} disabled={actionBusy || !selected?.complete || data?.job?.running || ['running', 'stopping'].includes(data?.runtime?.status)}><Navigation size={16} />导航</button></div>}
          </section>
        </div>
      </section>
        ...
        <aside className="sidebar">
          ...
          {section === 'sessions' && (
            <div className="item-list session-list">
              {data?.current_capture.available && <div className="current-capture"><div><Activity size={16} /><strong>最新原始 PCD</strong><span>{data.current_capture.archived ? '已保存' : '未保存'}</span></div><p>{data.current_capture.file.size_human} · {formatTime(data.current_capture.file.modified_at)}</p><button className="button secondary wide" onClick={() => setModal('archive')} disabled={data.current_capture.archived}><Archive size={15} />{data.current_capture.archived ? 'PCD 已在列表中' : '保存这个 PCD'}</button></div>}
              {data?.sessions.map((session) => (
                <div className="session-row" key={session.id}><div className="session-icon"><HardDrive size={18} /></div><div><strong>{session.name}</strong><span>{session.site} · {formatTime(session.created_at)}</span><code>{session.raw?.size_human || '文件缺失'}</code></div><div className="row-actions"><IconButton title="从该会话生成地图" onClick={() => setModal({ type: 'build', sessionId: session.id })}><ChevronRight size={17} /></IconButton><IconButton title="移到回收站" onClick={() => moveToTrash('session', session.id)} disabled={actionBusy}><Trash2 size={16} /></IconButton></div></div>
              ))}
            </div>
          )}
        ...
        <main className="preview-area">
          {(section === 'legacy' && selectedLegacy) ? (
            ...
            <>
              ...
              <div className="stage-footer"><span><MapPin size={14} />{selected.site}</span><span><Clock3 size={14} />{formatTime(selected.created_at)}</span><span><HardDrive size={14} />{selected.size_human}</span></div>
            </>
          ) : (
            <div className="empty-stage"><div className="empty-map-symbol"><Map size={34} /></div><h2>还没有地图版本</h2><div><button className="button secondary" onClick={() => setSection('legacy')}><History size={16} />查看旧地图</button><button className="button primary" onClick={() => startRuntime('mapping')}><Map size={16} />开始建图</button></div></div>
          )}
        </main>
        ...
        <aside className="inspector">
          {selected ? (
            <>
              ...
              <section className="detail-section"><h3>身份</h3><dl><div><dt>版本</dt><dd><code>{selected.id}</code></dd></div><div><dt>来源</dt><dd>{selected.origin}</dd></div><div><dt>建图记录</dt><dd>{selected.source_session || '历史导入'}</dd></div><div><dt>备注</dt><dd>{selected.note || '无'}</dd></div></dl></section>
              ...
              <div className="inspector-actions">
                {selected.status === 'active' ? <div className="active-confirm"><Check size={17} /><span><strong>当前正在使用</strong><small>2D 地图与 ICP PCD 已绑定</small></span></div> : <div className="active-confirm selection-confirm"><MapPin size={17} /><span><strong>已选择这套地图</strong><small>启动定位或导航时自动切换</small></span></div>}
          ...
          ) : <div className="inspector-empty"><SlidersHorizontal size={22} /><span>选择一个地图版本查看详情</span></div>}
        </aside>
      </div>
