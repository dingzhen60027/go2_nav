...
bool IMUProcessor::initialize(SyncPackage &package)
{
    ...
    const size_t required_samples = static_cast<size_t>(m_config.imu_init_num);
    if (m_imu_cache.size() > required_samples)
    {
        m_imu_cache.erase(m_imu_cache.begin(), m_imu_cache.end() - required_samples);
    }
    if (m_imu_cache.size() < required_samples)
    {
        m_initialization_status = "waiting for IMU samples (" + std::to_string(m_imu_cache.size()) +
    ...
    for (const auto &imu : m_imu_cache)
    {
        acc_variance += (imu.acc - acc_mean).squaredNorm();
        gyro_variance += (imu.gyro - gyro_mean).squaredNorm();
    }
    const double acc_std = std::sqrt(acc_variance / static_cast<double>(m_imu_cache.size()));
    const double gyro_std = std::sqrt(gyro_variance / static_cast<double>(m_imu_cache.size()));
    const double acc_norm = acc_mean.norm();
    if (!std::isfinite(acc_norm) || acc_norm < m_config.imu_init_acc_norm_min ||
        acc_norm > m_config.imu_init_acc_norm_max)
    {
        m_initialization_status = "invalid acceleration magnitude; keep MID360 stationary";
        return false;
    }
    if (!std::isfinite(acc_std) || !std::isfinite(gyro_std) ||
        acc_std > m_config.imu_init_acc_std_max || gyro_std > m_config.imu_init_gyro_std_max)
    {
        m_initialization_status = "MID360 is moving; waiting for a stationary IMU window";
        return false;
    }
