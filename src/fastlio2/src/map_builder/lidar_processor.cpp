...
void LidarProcessor::resizeWorkspaces(size_t point_count)
{
    m_cloud_down_world->resize(point_count);
    m_norm_vec->resize(point_count);
    m_effect_cloud_lidar->resize(point_count);
    m_effect_norm_vec->resize(point_count);
    m_nearest_points.resize(point_count);
    m_point_selected_flag.assign(point_count, 0);
}
...
void LidarProcessor::process(SyncPackage &package)
{
    ...
    resizeWorkspaces(m_cloud_down_lidar->size());
    trimCloudMap();
    m_kf->update();
    incrCloudMap();
}
...
#pragma omp parallel for
    ...
    if (effect_feat_num < m_config.min_effective_points)
    {
