...
bool RotationTrajectory::build(
    const std::deque<ImuSample> &imu_samples,
  ...
  const auto first_after_start = std::upper_bound(
      ...
      [](int64_t stamp, const ImuSample &sample) { return stamp < sample.stamp_ns; });
  ...
  const auto first_at_or_after_end = std::lower_bound(
      ...
      [](const ImuSample &sample, int64_t stamp) { return sample.stamp_ns < stamp; });
  ...
  const int64_t max_gap_ns = static_cast<int64_t>(max_imu_gap_sec / kNanosecondsToSeconds);
  for (auto current = coverage_begin; current != first_at_or_after_end; ++current) {
    const auto next = std::next(current);
    if (!current->angular_velocity.allFinite() || !next->angular_velocity.allFinite()) {
  ...
  auto angularVelocityAt = [&](int64_t stamp_ns) {
    ...
    if (right != imu_samples.end() && right->stamp_ns == stamp_ns) {
      return rotation_lidar_from_imu * (right->angular_velocity - gyro_bias);
    }
    ...
    const Eigen::Vector3d interpolated =
        left->angular_velocity + ratio * (right->angular_velocity - left->angular_velocity);
    return rotation_lidar_from_imu * (interpolated - gyro_bias);
  };

  std::vector<int64_t> knot_times;
  knot_times.reserve(static_cast<size_t>(std::distance(coverage_begin, first_at_or_after_end)) + 2);
  knot_times.push_back(scan_start_ns);
  for (auto sample = first_after_start; sample != first_at_or_after_end; ++sample) {
  ...
  for (size_t i = 1; i < knot_times.size(); ++i) {
    const int64_t previous_ns = knot_times[i - 1];
    const int64_t current_ns = knot_times[i];
    const double dt = static_cast<double>(current_ns - previous_ns) * kNanosecondsToSeconds;
    const Eigen::Vector3d average_angular_velocity =
        0.5 * (angularVelocityAt(previous_ns) + angularVelocityAt(current_ns));
    rotation = (rotation * rotationFromVector(average_angular_velocity * dt)).normalized();
...
Eigen::Quaterniond RotationTrajectory::rotationAt(int64_t stamp_ns) const {
  ...
  const auto right = std::lower_bound(
      ...
      [](const RotationKnot &knot, int64_t stamp) { return knot.stamp_ns < stamp; });
...
Eigen::Vector3d RotationTrajectory::compensate(const Eigen::Vector3d &point,
